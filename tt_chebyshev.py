    ...
    def _riemannian_cg_step(self, tt_tensor, indices, values, max_iter=20):
        ...
        for dim in range(self.d):
            ...
            for i, (idx, delta_i) in enumerate(zip(indices, delta)):
                ...
                if dim == 0:
                    # For the first core, left_part is just a scalar
                    left_part = np.array([[1.0]])
                else:
                    # Initialize with the first core
                    left_core = result.cores[0]
                    left_part = left_core[:, idx[0], :]
                    
                    # Contract with remaining cores up to dim-1
                    for d_left in range(1, dim):
                        left_core = result.cores[d_left]
                        left_part = left_part @ left_core[:, idx[d_left], :]
                
                if dim == self.d - 1:
                    # For the last core, right_part is just a scalar
                    right_part = np.array([[1.0]])
                else:
                    # Initialize with the last core
                    right_core = result.cores[-1]
                    right_part = right_core[:, idx[-1], :].T
                    
                    # Contract with remaining cores down to dim+1
                    for d_right in range(self.d-2, dim, -1):
                        right_core = result.cores[d_right]
                        right_part = right_core[:, idx[d_right], :].T @ right_part
                    
                    # Transpose to get correct shape
                    right_part = right_part.T
                
                # Update the core
                update = delta_i * (left_part @ right_part)
                current_core = result.cores[dim]
                flat_idx = (slice(None), idx[dim], slice(None))
                
                # Ensure update has the right shape
                if update.shape != current_core[flat_idx].shape:
                    # Reshape if needed
                    update = update.reshape(current_core[flat_idx].shape)
                
                # Apply update with small step size
                current_core[flat_idx] += update * 0.01
...
class TensorTrainChebyshevInterpolation:
    ...
    def _tt_inner_product(self, tt1, tt2):
        ...
        for d in range(tt1.order):
            ...
            # Create intermediate tensor by contracting over the mode dimension
            # Shape: (r1, r2, s1, s2) where r's are TT ranks of first tensor, s's are TT ranks of second tensor
            intermediate = np.zeros((core1.shape[0], core1.shape[2], core2.shape[0], core2.shape[2]))
            
            for i in range(core1.shape[1]):  # Loop over the mode dimension
                intermediate += np.outer(core1[:, i, :].reshape(-1), core2[:, i, :].reshape(-1)).reshape(
                    core1.shape[0], core1.shape[2], core2.shape[0], core2.shape[2])
            
            # Reshape the result for contraction with previous result
            intermediate = intermediate.reshape(core1.shape[0] * core1.shape[2], core2.shape[0] * core2.shape[2])
